...
touch(path.join(w.INBOX, 'a.MP3'));
touch(path.join(w.INBOX, 'b.WaV'));
touch(path.join(w.INBOX, 'c.AIF'));
const got3 = w.listPending().map(p => path.basename(p)).sort();
assert('MP3 + WAV + AIF all included', got3, ['a.MP3', 'b.WaV', 'c.AIF']);
...
// 9. AIFF parsing — exercises parseAiff() in songs-indexer.js
// against a real AIFF file. Verifies COMM chunk decoding
// (sampleRate, channels, frames) + duration math.
process.stderr.write(`9. AIFF parsing\n`);
try {
    const aiffPath = '/Users/kaidejuricmasscmbook/Downloads/adoresoundcloudver (2).aif';
    if (fs.existsSync(aiffPath)) {
        const { execFileSync } = require('node:child_process');
        execFileSync(process.execPath,
            [path.join(__dirname, 'songs-indexer.js'), '--add', aiffPath, '--apply', '--yes'],
            { encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] });
        const cat = JSON.parse(fs.readFileSync(
            path.join(__dirname, '..', 'data', 'songs', 'catalog.json'), 'utf8'));
        const aiff = cat.items.find(i => i.relPath && i.relPath.endsWith('.aif'));
        assertTrue('AIFF catalog entry exists', !!aiff);
        if (aiff) {
            assertTrue('AIFF format=aif', aiff.format, 'aif');
            assertTrue('AIFF sampleRate is 44100', aiff.sampleRate, 44100);
            assertTrue('AIFF channels is 2', aiff.channels, 2);
            assertTrue('AIFF durationSec > 50s', aiff.durationSec > 50 && aiff.durationSec < 70);
        }
    } else {
        console.log('  (skipped — fixture AIFF not present at ' + aiffPath + ')');
    }
} catch (e) {
    failed += 1;
    console.log(`  ✗ AIFF parse: ${e.message}`);
}
