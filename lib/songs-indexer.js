...
function index() {
  ...
  for (const root of cfg.roots) {
    ...
    if (!fs.existsSync(abs)) {
      rootSummary.push({ label: root.label, path: abs, found: 0, missing: true });
    ...
    for (const filePath of walk(abs)) {
      ...
      let parsed;
      try {
        parsed = ext === '.mp3' ? parseMp3(filePath) : parseWav(filePath);
      } catch (e) {
    ...
    rootSummary.push({ label: root.label, path: abs, found, missing: false });
  }
