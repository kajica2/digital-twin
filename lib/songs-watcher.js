...
// Supported extensions: .mp3, .wav, .aif, .aiff, .m4a, .aac,
// .flac, .ogg. The watcher drops audio files; the indexer parses
// their headers. AIFF support was added in sprint 0.35.
...
function lockFor(audioPath) {
    // Hash the path. Includes a per-watcher prefix so the songs-
    // watcher's .processing-* files never collide with the chart-
    // watcher's if a future refactor shares an inbox between them.
    const h = crypto.createHash('sha1')
        .update('songs-watcher:')
        .update(audioPath)
        .digest('hex')
        .slice(0, 12);
...
function listPending() {
    ...
    try { entries = fs.readdirSync(INBOX); } catch { return []; }
    const now = Date.now();
    const SUPPORTED = ['.mp3', '.wav', '.aif', '.aiff', '.m4a', '.aac', '.flac', '.ogg'];
    return entries
