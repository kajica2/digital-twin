...
const INBOX = path.join(REPO, 'chart-inbox');
...
// ---------- Lockfile management ----------
// Each .musicxml gets a lockfile during export. Stale lockfiles
// (older than 30 min) are pruned at startup so a crash doesn't
// permanently block a file.
const STALE_LOCK_MS = 30 * 60 * 1000;

function lockFor(musicxmlPath) {
    const h = crypto.createHash('sha1').update(musicxmlPath).digest('hex').slice(0, 12);
...
function exportOne(musicxmlPath) {
    ...
    try {
        ...
        appendLog(`[done]  ${path.basename(musicxmlPath)} → ${sec}s, moved to processed/`);
        return { ok: true, sec };
    } catch (e) {
        const sec = ((Date.now() - t0) / 1000).toFixed(1);
        const tail = (e.stderr || e.stdout || '').toString().split('\n').slice(-3).join(' | ');
        appendLog(`[fail]  ${path.basename(musicxmlPath)} after ${sec}s: ${e.message} :: ${tail}`);
        return { ok: false, sec, error: e.message };
    } finally {
        releaseLock(lock);
    }
}
...
function listPending() {
    ...
    return entries
        ...
        .filter(e => !fs.existsSync(lockFor(path.join(INBOX, e))))
...
function main() {
    ensureDirs();
    pruneStaleLocks();
    appendLog(`[boot]  chart-watcher started (interval=${INTERVAL_MS}ms once=${ONCE})`);
