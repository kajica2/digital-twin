...
async function submitToMidjourney({
    ...
    const outputBase = path.join(outputDir, slugify(prompt.slice(0, 40)));
    fs.mkdirSync(outputDir, { recursive: true });

    if (dryRun) {
        console.log(`[dry-run] Prompt: ${fullPrompt}`);
        console.log(`[dry-run] Channel: ${channelUrl}`);
        console.log(`[dry-run] Output: ${outputBase}.png`);
    ...
    try {
        ...
        let textareaInfo;
        try {
            textareaInfo = await cdp.waitForSelector(textareaSelector, 45000);
        } catch (e) {
            // Take a diagnostic screenshot
            const diagData = await cdp.screenshot();
            const diagPath = `${outputBase}_diagnostic.png`;
            fs.writeFileSync(diagPath, Buffer.from(diagData, 'base64'));
            throw new Error(`Discord textarea not found within 45s. Diagnostic screenshot saved to ${diagPath}`);
        }
        ...
        if (resultUrl) {
            ...
            const pngPath = `${outputBase}_result.png`;
            ...
            const metaPath = `${outputBase}_meta.json`;
            ...
            screenshotData = await cdp.screenshot();
            const timeoutPath = `${outputBase}_timeout.png`;
