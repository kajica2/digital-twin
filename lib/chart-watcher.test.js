...
// 14. post-success hook fires after successful export
process.stderr.write(`14. post-success hook fires on success\n`);
resetInbox();
const hookFile = path.join(w.INBOX, 'hook.musicxml');
touch(hookFile);
w.setRunExport(() => {});
let hookCalls = 0;
let hookArgs = null;
w.setPostSuccessHook((args) => {
    hookCalls += 1;
    hookArgs = args;
});
const r14 = w.exportOne(hookFile);
assert('export ok', r14.ok, true);
assertTrue('hook called once', hookCalls === 1);
assertTrue('hookArgs.songBase === "hook"', hookArgs && hookArgs.songBase === 'hook');
assertTrue('hookArgs.attempts === 1', hookArgs && hookArgs.attempts === 1);
assertTrue('hookArgs.sec parses as number', hookArgs && !isNaN(parseFloat(hookArgs.sec)));

// 15. post-success hook does NOT fire on retry-then-fail
process.stderr.write(`15. post-success hook does NOT fire on permanent fail\n`);
resetInbox();
const failHookFile = path.join(w.INBOX, 'failhook.musicxml');
touch(failHookFile);
w.setRunExport(() => { throw new Error('always fails'); });
let failHookCalls = 0;
w.setPostSuccessHook(() => { failHookCalls += 1; });
w.exportOne(failHookFile);
assertTrue('hook never called on permanent fail', failHookCalls === 0);

// 16. post-success hook failure does not break the export
process.stderr.write(`16. post-success hook failure is logged, does not break export\n`);
resetInbox();
const brokenHookFile = path.join(w.INBOX, 'brokenhook.musicxml');
touch(brokenHookFile);
w.setRunExport(() => {});
w.setPostSuccessHook(() => { throw new Error('hook itself broke'); });
const r16 = w.exportOne(brokenHookFile);
assert('export still ok despite hook failure', r16.ok, true);
assertTrue('input moved despite hook failure', fs.existsSync(path.join(w.PROCESSED, 'brokenhook.musicxml')));
