...
function main() {
    ...
    ordered.forEach((p, i) => {
        ...
        const singleXml = wrapSinglePart(xml, p.id);
        if (!singleXml) {
...
function renderAll(outPdfDir, outNoTrumpetDir, outNoSaxDir, songBase) {
    if (!which('mscore')) {
...
function runMscore(inputXml, outputPath, label) {
    ...
    process.stderr.write(`[render] ${label} (${size} bytes, ${sec}s)\n`);
}
