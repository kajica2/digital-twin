...
if (HELP) {
    process.stderr.write(`Usage: node lib/chart-export.js [path.musicxml] [options]

Options:
  --apply               Write outputs (default: dry-run summary)
  --yes                 Skip the y/N confirmation prompt
  --render              Post-export: invoke mscore to produce PDF + MP3
  --mp3-only            Skip splitting + muting, just re-render audio
                        (requires a previous --apply to have produced
                        <song>_Whole/, <song>_No_Trumpet/, <song>_No_Sax/)
  --chords <json-file>  Inject <harmony> elements from a sidecar JSON.
                        Format: { "1": "Dm7", "2": "G7sus", ... }
  --help, -h            Show this message

Path argument defaults to /tmp/mini-score.musicxml if no file given.
When --render runs, mscore must be on PATH (brew install --cask musescore).
`);
    process.exit(0);
}
if (!INPUT) {
...
// Cheap XML well-formedness check for a score about to be handed to
// MuseScore. Shared with lib/musicxml-to-pdf.js so there is one
// implementation — see lib/xml-guard.js for the rationale (mscore
// hangs rather than errors on truncated input).
...
function runMscore(inputXml, outputPath, label) {
    ...
    // Fail fast on malformed input instead of letting mscore hang.
    const wf = musicXmlIsWellFormed(inputXml);
    if (!wf.ok) {
        process.stderr.write(
            `[render] FAIL ${label}: ${inputXml} is not well-formed XML — ${wf.reason}.\n` +
            `[render]        Rejected before invoking mscore (it hangs rather than errors on bad input).\n`);
        return;
    }
    ...
    // the exit code. The timeout is a backstop for the hang above
    // (unbounded was the prior behaviour).
    const r = spawnSync('mscore', ['-f', inputXml, '-o', outputPath],
              { stdio: ['ignore', 'pipe', 'pipe'], timeout: 180000 });
    if (r.error && r.error.code === 'ETIMEDOUT') {
        process.stderr.write(`[render] FAIL ${label}: mscore timed out after 180s\n`);
        return;
    }
